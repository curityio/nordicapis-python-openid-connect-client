...
class Client:
    ...
    def revoke(self, token):
        ...
        if 'revocation_endpoint' not in self.server_config:
            raise Exception(
                'Server configuration did not contain a revocation endpoint')
    ...
    def refresh(self, refresh_token):
        ...
        try:
            ...
            raise Exception(
                "The refresh request needs to be completed. Server error: %s" %
                te.reason)
    ...
    def get_authorization_request_url(self, state):
        ...
        login_url = "%s?%s" % (self.server_config['authorization_endpoint'],
    ...
    def __validate_jwt(self, jwt, iss, aud):
        ...
        if iss != payload['iss']:
            raise Exception(
                "Invalid issuer %s, expected %s" % (payload['iss'], iss))
    ...
    def __load_keys(self):
        print("Loading keys from %s" % self.server_config['jwks_uri'])
        return jwkest.jwk.load_jwks_from_url(
            self.server_config['jwks_uri'])
    ...
    def __get_server_config(self):
        ...
        if 'issuer' in self.client_config:
            discovery_url = self.client_config[
